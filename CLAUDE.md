...
│   ├── smoke-apa.mjs               # Apa v1: Section A = occupational_assessments schema; Section B = full E2E (needs Next running, no edge fns — no LLM)
...
node scripts/create-org.mjs <SUPABASE_URL> <SERVICE_KEY> --type program --name "Dhaka Urban MCH Program" --signer-email s@program.bd --signer-password <pw> --signer-name "Dr. Signatory" --staff-email d@program.bd --staff-password <pw> --staff-name "CHW Doctor"
  # Supported org types: diagnostic_centre, hospital, employer, recruiter, program. clinic refused (use create-doctor.mjs). Signatory role is always 'signatory'; staff role is doctor (hospital/employer/recruiter/program) or technologist (centre).
