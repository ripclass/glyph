...
*Last updated: 2026-06-21 — **PROD DB IS NOW AT MIGRATION 018.** `supabase db push` applied 013→018 in one shot (prod had been at 012; the Hospital/Karigor/Continuity/Maa/Bridge module schemas — 013–017 — had never been pushed and rode along with 018, all additive). So `discharge_records`, `occupational_assessments`, `clearance_records`, `antenatal_visits` (+`program` org_type), `specialist_opinions` (+`specialist_panel` org_type), and the emergency-access tables now all exist on prod (deny-all/member RLS; no surfaces exposed until the relevant frontend deploys). **Emergency Access v1 + self-SOS are MERGED & LIVE ON PROD** (both ship "dark" — inert until a patient opts in and data exists). (a) **Emergency Access v1** (lock-screen QR + stranger-scan + 10km hospital broadcast + family ping; migration 018, `emergency-logic`/`emergency.ts`, `/e/[token]` public scan, wallet emergency card) — PR #24 merged, Vercel prod deploy green, route verified inert (`GET /api/e/<unknown>` → 404). (b) **Emergency self-SOS Phase 1** (a WhatsApp-bound patient texts `SOS` → "share location to confirm" → shares location → fires the SAME engine; no migration, engine untouched; `location` parser kind + `isSosWord` intent + preemptive router `sos_prompt`/`sos_fire`/`sos_cancel` + `ensureEmergencyEnabled` + `smoke-sos.mjs`) — PR #26 merged, prod deploy green. Spec/plan: `docs/superpowers/{specs,plans}/2026-06-21-emergency-self-sos*`. Trigger-layer remaining phases: Pocket SOS button (Phase 2), proxied stranger call number (Phase 3, needs telephony). Before either feature actually protects anyone (tracked on **GitHub issue #25**): 2 Meta-approved WA templates (`glyph_emergency_family`/`glyph_emergency_hospital`), governance/clinical/legal review, and prod hospital orgs need real `latitude`/`longitude` (currently ZERO hospital orgs on prod, so the broadcast no-ops). Verified local: 222 tests green, `smoke-emergency.mjs` 19/19, `smoke-sos.mjs` node-checked (live run deferred). Prior (2026-06-19): owner/scope migration 011; Lens v1 (012 + /center + lens-normalize); KhaM-Med self-hosted adapter DORMANT until `MEDGEMMA_BASE_URL` set; `config.toml` pins `verify_jwt=false` for triage/extract-document/lens-normalize. Prod gates green: smoke-path 19/19, smoke-documents 16/16. Keep this file current. If a future session needs something and finds it missing here, that's a signal to add it.*
