...
*Last updated: 2026-06-21 — **PROD DB IS NOW AT MIGRATION 018.** `supabase db push` applied 013→018 in one shot (prod had been at 012; the Hospital/Karigor/Continuity/Maa/Bridge module schemas — 013–017 — had never been pushed and rode along with 018, all additive). So `discharge_records`, `occupational_assessments`, `clearance_records`, `antenatal_visits` (+`program` org_type), `specialist_opinions` (+`specialist_panel` org_type), and the emergency-access tables now all exist on prod (deny-all/member RLS; no surfaces exposed until the relevant frontend deploys). **Emergency Access v1** (lock-screen QR + stranger-scan + 10km hospital broadcast + family ping; migration 018, `emergency-logic`/`emergency.ts`, `/e/[token]` public scan, wallet emergency card) is built on branch `emergency-access-v1` / **PR #24 — NOT yet merged to `main`, so its frontend is NOT live on prod.** Verified local: type-check + 211 tests green, `smoke-emergency.mjs` 19/19. Before patient-facing: 2 Meta-approved WA templates (`glyph_emergency_family`/`glyph_emergency_hospital`), governance/clinical/legal review, then merge PR #24. Prior (2026-06-19): owner/scope migration 011; Lens v1 (012 + /center + lens-normalize); KhaM-Med self-hosted adapter DORMANT until `MEDGEMMA_BASE_URL` set; `config.toml` pins `verify_jwt=false` for triage/extract-document/lens-normalize. Prod gates green: smoke-path 19/19, smoke-documents 16/16. Keep this file current. If a future session needs something and finds it missing here, that's a signal to add it.*
