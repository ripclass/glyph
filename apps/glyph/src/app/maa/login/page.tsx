...
export default function MaaLoginPage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      if (!staff || !requireOrgType(staff, 'program')) {
        toast.error('Signed in, but this account is not a program member.');
        return;
      }
  ...
  return (
    <div className="flex min-h-screen items-center justify-center bg-clinical-bg p-4">
      <form onSubmit={handleSubmit} className="w-full max-w-sm space-y-4 rounded-xl border border-line bg-white p-6">
        <h1 className="text-lg font-semibold text-ink">Glyph Maa sign in</h1>
