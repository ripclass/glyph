...
function MaaChrome({ children }: { children: React.ReactNode }) {
  const staff = useStaffStore((s) => s.staff);
  const signOut = useStaffStore((s) => s.signOut);
  return (
    <div className="min-h-screen bg-clinical-bg">
      <header className="flex items-center justify-between border-b border-line bg-white px-4 py-3">
        <Link href="/maa" className="font-semibold text-ink">{staff?.orgName ?? 'Glyph Maa'}</Link>
...
function MaaGuard({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    // If session loaded but no staff, or staff exists but wrong org type → redirect to maa login.
    if (checkedRef.current && !isLoading) {
      if (!staff || !requireOrgType(staff, 'program')) {
        router.replace('/maa/login');
      }
    }
  }, [isLoading, staff, router]);

  if (isLoading || !staff || !requireOrgType(staff, 'program')) {
    return (
  ...
  return <MaaChrome>{children}</MaaChrome>;
}

export default function MaaLayout({ children }: { children: React.ReactNode }) {
  const pathname = usePathname();
  // Exempt the login page from MaaGuard to prevent a redirect loop.
  if (pathname === '/maa/login') {
    return <>{children}</>;
  }
  return <MaaGuard>{children}</MaaGuard>;
}
